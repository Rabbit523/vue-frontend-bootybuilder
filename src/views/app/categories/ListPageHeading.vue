  ...
  <b-row>
    <b-colxx xxs="12">
      ...
      <div class="top-right-button-container d-flex">
        <!-- <div class="top-right-button d-flex align-items-center">
          <switches v-model="qr_code" theme="custom" color="primary" class="mb-0" @click="onShowQrCode()"></switches>
          <a class="ml-1 mr-2 mb-0" style="font-size: 18px;">QR code</a>
        </div> -->
        <router-link to="add-new-category">
          <b-button
            @click="onAddNew()"
            variant="primary"
            size="lg"
        ...
        <b-button-group>
          <b-dropdown split right @click="selectAll(true)" class="check-button" variant="primary">
            ...
            <b-dropdown-item @click="onContextMenuAction('delete')">{{$t('pages.delete')}}</b-dropdown-item>
          </b-dropdown>
        </b-button-group>
      </div>
...
export default {
  ...
  watch: {
    qr_code() {
      this.setShowQrCodeChecked(this.qr_code);
    }
  },
  methods: {
    onAddNew() {
