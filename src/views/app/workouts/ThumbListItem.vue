...
<b-card @click.prevent="toggleItem($event,data.id)" :class="{'d-flex flex-row':true,'active' : selectedItems.includes(data.id)}" no-body>
    <router-link :to="`manage-workout?id=${data.id}`" class="d-flex">
    ...
    <div class="pl-2 d-flex flex-grow-1 min-width-zero">
        <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
            <router-link :to="`manage-workout?id=${data.id}`" class="w-30 w-sm-100">
                <p class="list-item-heading mb-0 truncate">{{data.title}}</p>
            </router-link>
            <p class="mb-0 text-muted text-small w-15 w-sm-100">{{data.amount_weeks_program}} TIMES PER WEEK</p>
            ...
            <div class="w-15 w-sm-100 text-center">
                <b-badge pill :variant="parseInt(data.subscribed)?'success':'primary'">{{parseInt(data.subscribed)?'Subscribed':'Unsubscribed'}}</b-badge>
            </div>
