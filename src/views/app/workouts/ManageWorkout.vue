        ...
        <b-colxx xxs="12" md="12" xl="8" class="col-left">
          ...
          <b-row class="mb-2" style="padding-left: 20px; padding-right: 20px;">
            <div class="w-100 d-flex justify-content-between align-items-center">
              <h3>Exercises</h3>
              <div>
                <b-button v-if="weekdays.length" class="mb-1 block mr-1" variant="secondary" @click="onDeleteTab()">Delete Weekday</b-button>
          ...
          <b-card v-if="weekdays.length" class="mb-4" no-body>
            <b-tabs card no-fade v-model="tabIndex">
              <b-modal id="modalEditTab" ref="modalEditTab" title="Edit Weekday">
                  <b-form-group label-cols="2" horizontal label="Weekday Name">
                    <b-form-input v-model="editTag" placeholder="Weekday Name"></b-form-input>
                  </b-form-group>
                  <template slot="modal-footer">
                      <b-button variant="primary" @click="changeTagName('modalEditTab')" class="mr-1">Update</b-button>
          ...
          <b-card class="mb-4" no-body>
            <b-card-body>
              ...
              <b-row class="align-items-center m-0">
                  <switches v-model="subscribed" theme="custom" color="primary" class="mb-2"></switches>
...
export default {
  ...
  methods: {
    ...
    publish(is_published) {
      ...
      axios.post(apiUrl+'/update_workouts', formdata,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
    ...
    onDeleteTab() {
      ...
      .then((result) => {
        if (result.value) {
          var id = this.weekdays[this.tabIndex].id;
          ...
          if(id >= 0) {
            var token = localStorage.getItem("token");
            axios.get(apiUrl+'/delete_weekday?weekday_id='+id, {
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`
              }
    ...
    loadItems(id) {
      ...
      axios
        .get(apiUrl+'/get_workout_content?id='+id, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
          }
        ...
        .then(res => {
          ...
          this.subscribed = parseInt(res.data.subscribed);
          this.has_timer = parseInt(res.data.has_timer);
          this.productID = res.data.productId;
          this.weekdays = res.weekdays ? res.weekdays : [];
          this.amount_weeks_program = parseInt(res.data.amount_weeks_program);
          this.addedExercises = res.weekdays && res.weekdays.length > 0 ? res.weekdays[0].exercises : null;
          this.level1_work = parseInt(res.data.level1_work);
          this.level1_rest = parseInt(res.data.level1_rest);
          this.level1_rounds = parseInt(res.data.level1_rounds);
          this.level2_work = parseInt(res.data.level2_work);
          this.level2_rest = parseInt(res.data.level2_rest);
          this.level2_rounds = parseInt(res.data.level2_rounds);
          this.level3_work = parseInt(res.data.level3_work);
          this.level3_rest = parseInt(res.data.level3_rest);
          this.level3_rounds = parseInt(res.data.level3_rounds);
          if (res.data.category) this.category = res.data.category;
          this.isLoad = true;
        });
    },
